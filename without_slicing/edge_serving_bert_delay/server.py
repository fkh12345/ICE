...
#model = Yolov3()
model = My_NER()
data = torch.rand(1, 32, 768)
#data = torch.rand(1, 3, 299, 299)
#data = torch.rand(1, 1, 400, 200)
DAG, DAG_layer_name = model._trace_graph(data)
print(DAG_layer_name)
...
class FormatData(data_pb2_grpc.FormatDataServicer):
    def DoFormat(self, request, context):
        ...
        serving.prepare_data(start, end, input, query_id)
    ...
    try:
        while True:
            ...
            if(len(serving.input) == 0):
                ...
                continue
            if(args.method == 'ICE'):
                serving.queue_time = change_waiting_queue(serving.start, serving.queue_time_origin, [0, 23])
