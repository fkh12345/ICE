...
class FormatData(data_pb2_grpc.FormatDataServicer):
    def DoFormat(self, request, context):
        ...
        index = serving.push_data(start, str, [0, 40, 101], "cuda:0")
        ...
        if(start > 0):
            serving.push_queue(start1, 0.045, 0.045)
        if(start > 70):
    ...
    try:
        while True:
            ...
            if(len(serving.input) == 0):
                ...
                continue
            serving.queue_time = change_waiting_queue(serving.start, serving.queue_time_origin, [0, 40, 101])
