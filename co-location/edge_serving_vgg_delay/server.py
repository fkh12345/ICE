...
class FormatData(data_pb2_grpc.FormatDataServicer):
    def DoFormat(self, request, context):
        ...
        index = serving.push_data(start, str, [0, 28, 40], "cuda:0")
        ...
        if(start > 0):
            serving.push_queue(start1, 0.05, 0.05)
        elif(start > 30):
            serving.push_queue(start1, 0.09, 0.09)
        else:
            serving.push_queue(start1, 0.08, 0.08)
...
def serve():
    grpcServer = grpc.server(futures.ThreadPoolExecutor(max_workers=300))  
    ...
    try:
        while True:
            ...
            if(len(serving.input) == 0):
                ...
                continue
            serving.queue_time = change_waiting_queue(serving.start, serving.queue_time_origin, [0, 28, 40])
