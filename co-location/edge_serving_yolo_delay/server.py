...
class FormatData(data_pb2_grpc.FormatDataServicer):
    def DoFormat(self, request, context):
        ...
        index = serving.push_data(start, str, [0, 85, 96], "cuda:0")
        ...
        if(start > 0):
            serving.push_queue(start1, 0.04, 0.04)
        else:
...
def serve():
    grpcServer = grpc.server(futures.ThreadPoolExecutor(max_workers=64))  
    ...
    try:
        while True:
            ...
            if(len(serving.input) == 0):
                ...
                continue
            serving.queue_time = change_waiting_queue(serving.start, serving.queue_time_origin, [0, 85, 96])
