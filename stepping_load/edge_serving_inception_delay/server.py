...
class FormatData(data_pb2_grpc.FormatDataServicer):
    def DoFormat(self, request, context):
        ...
        index = serving.push_data(start, str, [0, 101], "cuda:0")
...
def serve():
    grpcServer = grpc.server(futures.ThreadPoolExecutor(max_workers=3000))  
    ...
    try:
        while True:
            ...
            if(len(serving.input) == 0):
                ...
                continue
            if(args.method == 'ICE'):
                serving.queue_time = change_waiting_queue(serving.start, serving.queue_time_origin, [0, 101])
