...
class FormatData(data_pb2_grpc.FormatDataServicer):
    def DoFormat(self, request, context):
        ...
        index = serving.push_data(start, str, [0, 119], device)
        ...
        #print(end1 - start1)
        #print(serving.start)
        client_end = time.time() - server_start
        return data_pb2.actionresponse(text=out_time - start1, queue=out_time - launch, result=client_end)  
...
def serve():
    ...
    try:
        while True:
            #exit_tmp = os.popen('cat exit')
            #exit_tmp = int(exit_tmp.readline())
            #if(exit_tmp == 1):
            #    break
            time.sleep(0.005)
            #se.acquire()
